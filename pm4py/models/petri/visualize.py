...
def graphviz_visualization1(net,marking):
    ...
    viz.attr('node', shape='circle', fixedsize='true', width='0.75')
    for p in net.places:
        markingcount = marking[p]
        nodename = str(p.name) + str(markingcount)
        viz.node(str(p.name), str(p.name), xlabel=str(markingcount))
